...
namespace Genbox.VelcroPhysics.MonoGame.Samples.Testbed
{
    public sealed class Game1 : Game
    {
        ...
        {
            _test.DebugView.DrawString(
                50,
                15,
                _test.GetType().Name.Replace("Test", string.Empty, StringComparison.OrdinalIgnoreCase) +
                " / " +
                "R = reset / P = pause / I = prev test / O = next test / Home = reset view / F1 = shapes / F2 = debug panel / F3 = perf graph / F4 = AABB / F5 = center of mass / F6 = joints / F7 = contact points & normals / F8 = polygon points / F9 = polygon points / Cursors = pan / +/- = zoom"
            );
